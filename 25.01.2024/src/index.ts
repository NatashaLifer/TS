...
function stickPrice(clothes: Clothes[], marks: PriceMark[]): void {
...
function chengePriceforSale(clothes: Clothes[], clothesActie: string, discount: number): void {
    clothes.forEach(item => {
    ...
    structuredGoods.forEach(item => {
        ...
        ourSizes.forEach(singleSize => {
            if (!allSize.includes(singleSize)){
                ...
                const messageWrap: HTMLDivElement = document.querySelector('.message') as HTMLDivElement
                messageWrap.classList.remove('deactiveted')

                const orderClothes: HTMLElement = document.createElement('p')
...
const parentElement: HTMLElement = document.querySelector(".table__body") as HTMLElement
const CardComponent = (data: Clothes[], parent: HTMLElement): void => {
    data.forEach((item) => {
