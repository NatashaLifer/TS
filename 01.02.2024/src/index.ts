...
let selectedFreshProducts: Products[] = []
function showFreshProducts(prod: Products[]):Products[] {
    let dateNow: Date = new Date()

    selectedFreshProducts = prod.filter((singleProd: Products) => {
...
function sortProductsByPrice(prod: Products[]):Products[] {
...
function sortByCategories(prod: Products[]): ICategories {
    let structuredByCategories = prod.reduce((acc: ICategories, elem) => {
...
const parentElement: HTMLElement = document.querySelector(".wrapper") as HTMLElement
const renderCardProduct = (data: Products[], parent: HTMLElement): void => {
    data.forEach((item) => {
...
function checkGmoAndGliten(prod: Products[]): void {
    prod.forEach((singleProduct: Products) => { 
